...
create index if not exists epi_deliveries_group_idx
  on public.epi_deliveries(delivery_group_id, delivered_at desc);
...
begin
  ...
  loop
    ...
    insert into public.epi_deliveries(
      ...
      v_batch.brand_model, v_batch.lot_number, nullif(trim(p_note), '')
    );
  end loop;
