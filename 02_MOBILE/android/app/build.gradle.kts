...
// Never generate a distributable APK without the permanent update certificate.
gradle.taskGraph.whenReady {
    if (allTasks.any { it.project == project && it.name in listOf("assembleRelease", "bundleRelease", "packageRelease") }) {
        check(hasReleaseSigning) {
            "Metallo release exige metallo-release.jks e as credenciais ANDROID_KEYSTORE_PASSWORD, ANDROID_KEY_ALIAS e ANDROID_KEY_PASSWORD. Use o workflow oficial; não substitua a chave."
        }
    }
}
