...
import '../02_COMPONENTES/ui_action_lock.dart';
...
class AppUpdateService {
  ...
  static AppUpdateInfo? _parseManifest(
    ...
    if (url == null ||
        ...
        url.hasFragment ||
        url.path !=
            '/gunswiz/Metallo/releases/download/v$version/Metallo.apk') {
      if (strict) {
    ...
    try {
      ...
      final accepted = await showDialog<bool>(
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: Text(
              'A versão ${availableUpdate.version} (build ${availableUpdate.build}) está disponível. '
              'O download oficial será aberto no navegador. Depois, abra o APK baixado '
              'e confirme a atualização no Android. Não desinstale a versão atual. '
              'Se houver um bloqueio do Google, anote a mensagem e procure o responsável pelo Metallo.'),
          actions: [
            TextButton(
