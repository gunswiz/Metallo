...
export default async function NewMovementPage({ searchParams }: { searchParams: Promise<{ error?: string; asset?: string; item?: string; type?: string }> }) {
  const profile = await requireCapability("operations:write");
  const materialsAllowed=can(profile,"materials:write"), consumptionAllowed=can(profile,"consumption:write");
  ...
  return (
    <>
      <PageHeader eyebrow="REGISTRO DE MOVIMENTAÇÃO" title="Nova movimentação" description="Registre a movimentação de materiais e equipamentos. Para informar outra data ou trabalhar sem internet, use Obras e pedidos." />
      {query.error && <div className="alert error" role="alert">{errors[query.error] ?? errors.falha}</div>}
      <section className="content-grid">
        {(materialsAllowed||consumptionAllowed)&&<div className="panel"><header className="panel-header"><div><h2>Material</h2><p>Entrada, saída, transferência, consumo ou reposição</p></div></header><div className="panel-body">
          <form action={registerMaterialMovement} className="form-grid">
            ...
            <label>Operação<select name="movementType" defaultValue={!materialsAllowed || (query.type === "consumption" && consumptionAllowed) ? "consumption" : "transfer"}>{materialsAllowed&&<option value="entry">Entrada</option>}{materialsAllowed&&<option value="exit">Saída</option>}{materialsAllowed&&<option value="transfer">Transferência</option>}{materialsAllowed&&<option value="return">Devolução</option>}{consumptionAllowed&&<option value="consumption">Consumo</option>}{materialsAllowed&&<option value="replenishment">Reposição</option>}</select></label>
            <label>Quantidade<input name="quantity" type="number" min="1" defaultValue="1" required /></label>
            <label>Origem<select name="originTeamId" defaultValue=""><option value="">Externa / não se aplica</option>{teams.filter(team=>canOperateTeam(profile,team.id)).map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label>
        ...
        {can(profile,"equipment:write")&&<div className="panel"><header className="panel-header"><div><h2>Equipamento</h2><p>Transferência e mudança individual de situação</p></div></header><div className="panel-body">
          <form action={registerAssetMovement} className="form-grid">
            <label className="full">Patrimônio<select name="assetId" required defaultValue={query.asset ?? ""}><option value="" disabled>Selecione</option>{assets.data.filter(asset=>canOperateTeam(profile,asset.team_id)).map((asset) => <option key={asset.id} value={asset.id}>{asset.items?.name} · {asset.asset_code} · {asset.ownership_type === "rented" ? "Alugado" : "Próprio"}</option>)}</select></label>
