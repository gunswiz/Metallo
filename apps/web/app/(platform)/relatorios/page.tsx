...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <>
    ...
    <section className="panel analytics-filter"><form className="panel-body toolbar"><label className="compact-field">De<input name="from" type="date" lang="pt-BR" defaultValue={query.from ?? inputDate(range.currentStart)} /></label><label className="compact-field">Até<input name="to" type="date" lang="pt-BR" defaultValue={query.to ?? inputDate(new Date(range.currentEnd.getTime() - 86_400_000))} /></label><select className="filter-select" name="team" defaultValue={teamId ?? ""} aria-label="Filtrar por equipe"><option value="">Todas as equipes</option>{teams.map((team) => <option value={team.id} key={team.id}>{team.name}</option>)}</select><button className="button secondary" type="submit">Gerar relatório</button></form></section>
